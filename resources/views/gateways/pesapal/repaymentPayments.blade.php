...
<script>
    function makeRepaymentWithPesapal() {
        var iframe = document.createElement('iframe');
        iframe.src = "https://www.pesapal.com/payments?amount={{ $amount }}&email={{ auth()->user()->email }}&type=repayment";
        iframe.width = "100%";
        iframe.height = "600";
        ...
        window.addEventListener('message', function(event) {
            if (event.data === 'pesapal_payment_complete') {
                ...
                fetch("{{ route('completeRepayment', $amount) }}", {
                    method: "POST",
                    body: formData,
                })
